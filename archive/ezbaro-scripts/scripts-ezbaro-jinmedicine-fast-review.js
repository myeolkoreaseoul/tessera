...
const TARGET={taskNo:'RS-2025-02263117',ieNm:'진메디신',year:'2025',agency:'한국보건산업진흥원'};
...
async function ensureDetail(page){
  ...
  // search by task no only in cal00201 and pick ie/year row
...
(async()=>{
  ...
  try{
    ...
    const before=await page.evaluate(()=>{
      ...
      return {ok:true,taskNo:String(app?.gdsSelTask?.getColumn?.(0,'newTakN')||''),ieNm:String(app?.gdsSelTask?.getColumn?.(0,'ieNm')||''),rows,sum};
    });
    if(!before.ok) throw new Error(before.reason||'before fail');

    const forProfit=/\(주\)|주식회사/.test(before.ieNm||'');
    const unc=before.rows.filter(r=>r.status==='미확정');
    ...
    for(const r of unc){
      ...
      if(inKind){
        normal.push({...r,decision:'정상',reason:'집행방법 현물 규칙 적용'});
        continue;
      }

      if(forProfit && supply>0 && transfer>supply){
        weak.push({...r,decision:'미흡',reason:'영리기관 부가세 포함 집행(연구비이체금액>공급금액)'});
        continue;
      }

      // 연구재료비: 증빙 미등록은 미흡, 그 외는 인적검토 대상으로 유지
      if(isMat && fileCnt===0){
        weak.push({...r,decision:'미흡',reason:'연구재료비 필수 증빙 확인 불가(fileCount=0)'});
        continue;
      }

      keep.push({...r,decision:'미확정유지',reason:'명확한 미흡 사유 부재 또는 증빙 본문 검토 필요'});
    }
    ...
    const after=await page.evaluate(()=>{
      ...
      const ds=f2.ds_calOrdtmChckExeList; const rc=ds?.getRowCount?.()||0;
      const rows=[];
      for(let i=0;i<rc;i++) rows.push({rn:String(ds.getColumn(i,'rn')||''),decsnSn:String(ds.getColumn(i,'decsnSn')||''),title:String(ds.getColumn(i,'decsnTilTt')||''),status:String(ds.getColumn(i,'ordtmChckSuNm')||''),cpl:String(ds.getColumn(i,'ordtmChckCplSuNm')||'')});
      const sum={total:rows.length,unconfirmed:rows.filter(r=>r.status==='미확정').length,normal:rows.filter(r=>r.status==='정상').length,weak:rows.filter(r=>r.status==='미흡').length,reviewing:rows.filter(r=>r.cpl==='검토중').length,done:rows.filter(r=>r.cpl==='점검완료').length};
      return {ok:true,sum,rows};
    });

    const stamp=ts().replace(/[:.]/g,'-');
    const outJson=`projects/이지바로-진메디신-신속검토-${stamp}.json`;
    const outXlsx=`projects/이지바로-진메디신-신속검토-${stamp}.xlsx`;

    const logs=[...normal,...weak,...keep].map(r=>({전문기관:TARGET.agency,과제번호:TARGET.taskNo,수행기관:before.ieNm,순번:r.rn,결의순번:r.decsnSn,결의제목:r.title,비목:r.titNm,세목:r.sePpoNm,증빙유형:r.proofNm,fileCount:r.fileCount,공급금액:r.splAt,연구비이체금액:r.exeSplAt,처리결론:r.decision,처리사유:r.reason,처리시각:ts()}));

    const result={target:TARGET,resolved:{taskNo:before.taskNo,ieNm:before.ieNm},before:before.sum,plan:{normal:normal.length,weak:weak.length,keep:keep.length},applyNormal,applyWeak,confirm,after:after.sum,logs};
    fs.writeFileSync(outJson,JSON.stringify(result,null,2),'utf-8');
    const wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(logs),'판정로그');
    ...
    console.log(JSON.stringify({ok:true,outJson,outXlsx,result:{before:result.before,plan:result.plan,applyNormal,applyWeak,confirm,after:result.after}},null,2));
  } finally {
