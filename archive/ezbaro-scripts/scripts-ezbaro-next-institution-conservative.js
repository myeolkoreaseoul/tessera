...
const TARGET={taskNo:'RS-2025-02220553',ieNm:'대구한의대학교산학협력단',agency:'한국보건산업진흥원'};
const now=()=>new Date().toISOString();
const parseN=v=>Number(String(v||'').replace(/[^0-9-]/g,''))||0;
...
(async()=>{
 ...
 try{
  ...
  const open=await page.evaluate(({taskNo,ieNm})=>{
    ...
    if(idx<0) return {ok:false,reason:'target not found',rc};
  ...
  const before=await page.evaluate(()=>{
    ...
    const rows=[];
    for(let i=0;i<rc;i++) rows.push({idx:i,rn:String(ds.getColumn(i,'rn')||''),decsnSn:String(ds.getColumn(i,'decsnSn')||''),title:String(ds.getColumn(i,'decsnTilTt')||''),titNm:String(ds.getColumn(i,'titNm')||''),sePpoNm:String(ds.getColumn(i,'sePpoNm')||''),exeMt:String(ds.getColumn(i,'exeMtCCd')||''),proofNm:String(ds.getColumn(i,'proofNm')||''),fileCount:String(ds.getColumn(i,'fileCount')||''),amount:String(ds.getColumn(i,'rscpSeAt')||''),status:String(ds.getColumn(i,'ordtmChckSuNm')||''),cpl:String(ds.getColumn(i,'ordtmChckCplSuNm')||'')});
    return {ok:true,taskNo:String(app?.gdsSelTask?.getColumn?.(0,'newTakN')||''),ieNm:String(app?.gdsSelTask?.getColumn?.(0,'ieNm')||''),rows};
  });
  ...
  for(const r of targets){
    ...
    const fileCnt=parseN(r.fileCount);
    if(inKind) normal.push({...r,decision:'정상',reason:'집행방법 현물 규칙 적용(증빙 부재 허용)'});
    else if(fileCnt===0) weak.push({...r,decision:'미흡',reason:'증빙서류 미등록(fileCount=0)으로 필수 증빙 확인 불가'});
    else keep.push({...r,decision:'미확정유지',reason:'자동판정 기준상 명확 사유 부족(인적검토 필요)'});
  }

  const applyStatus=async (decsns,label)=>{
  ...
  const confirm=await page.evaluate(()=>{
    ...
    for(let i=0;i<rc;i++){
      ...
      ds.setColumn(i,'gridcmmcheck',hit?'1':'0'); if(hit) selected++;
    }
  ...
  const base=`projects/이지바로-${TARGET.ieNm}-자동정산-${stamp}`;
  const outJson=`${base}.json`; const outXlsx=`${base}.xlsx`;

  const logRows=[...normal,...weak,...keep].map(r=>({전문기관:TARGET.agency,과제번호:TARGET.taskNo,수행기관:TARGET.ieNm,순번:r.rn,결의순번:r.decsnSn,결의제목:r.title,비목:r.titNm,세목:r.sePpoNm,증빙유형:r.proofNm,fileCount:r.fileCount,처리결론:r.decision,처리사유:r.reason,처리시각:now()}));
  const result={target:TARGET,open,before:{sum:{total:before.rows.length,unconfirmed:targets.length}},plan:{normal:normal.length,weak:weak.length,keep:keep.length},applyNormal,applyWeak,confirm,after,logs:logRows};
  ...
  console.log(JSON.stringify({ok:true,outJson,outXlsx,result:{before:result.before,plan:result.plan,applyNormal,applyWeak,confirm,after:after.sum}},null,2));
