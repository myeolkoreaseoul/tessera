 ...
 */
export class GuidelineAnalyzer {
  ...
  async analyze(_pdfPath?: string): Promise<Guideline> {
    logger.info('지침 규정 로드 (2025년 책임의료기관 통합 사업 안내)');
  ...
  private printRules(g: Guideline) {
    logger.divider();
    logger.info(`사업 유형: ${g.projectType}`);
    logger.info(`비목 규정: ${Object.keys(g.rules).length}개`);
    logger.info(`제한 사항: ${g.restrictions.length}개`);
    logger.divider();
  }
}
