...
exports.InspectionJudge = void 0;
...
class InspectionJudge {
    config;
    guideline;
    constructor(config, guideline) {
        this.config = config;
        this.guideline = guideline;
    }
    async judge(record, attachments) {
    ...
    checkAllRules(record, attachments) {
        const issues = [];
        // 1. 첨부파일 없음
        ...
        if (categoryRule) {
            ...
            for (const required of categoryRule.required) {
                const found = attachedTypes.some(t => t.includes(required) || required.includes(t));
                if (!found) {
        ...
        // 5. 간이영수증 확인
        const hasSimpleReceipt = attachments.some(a => a.analysisResult?.documentType === '간이영수증');
        ...
        // 6. 문서 유형 불명
        const unknownDocs = attachments.filter(a => a.analysisResult?.documentType === '기타');
    ...
    findCategoryRule(budgetCategory) {
    ...
    generateOpinion(record, attachments, issues, judgment) {
    ...
    generateReasoning(record, attachments, issues) {
        const parts = [];
    ...
    async judgeAll(records, attachmentMap) {
        logger_1.logger.info(`${records.length}건 판단 시작...`);
        const results = [];
        for (let i = 0; i < records.length; i++) {
            const record = records[i];
            const attachments = attachmentMap.get(record.id) || [];
            logger_1.logger.progress(i + 1, records.length, `${record.budgetCategory}`);
...
exports.InspectionJudge = InspectionJudge;
