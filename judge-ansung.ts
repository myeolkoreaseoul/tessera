/**
 * 경기도의료원 안성병원 187건 검증 프레임워크 적용
 * ansung-data.json → 비목별 규칙 적용 → 판단 결과 JSON + 엑셀
 */
import fs from 'fs';
import ExcelJS from 'exceljs';

const DATA_JSON = '/mnt/c/projects/e-naradomum-rpa/ansung-data.json';
...
// ===== docType 분리 (복합 docType 대응) =====
function expandDocTypes(files: FileInfo[]): string[] {
  const types: string[] = [];
  for (const f of files) {
    // "지출결의서+세금계산서" → ["지출결의서", "세금계산서"]
    for (const part of f.docType.split('+')) {
      types.push(part);
    }
  }
  return types;
...
function commonChecks(rec: Record, j: Judgment): void {
  ...
  const docTypes = expandDocTypes(rec.files);
  ...
  for (const kw of forbidden) {
    if (rec.purpose.includes(kw) || rec.itemName.includes(kw)) {
      ...
      // 증빙 본문에서 발견 - OCR 오인식 패턴 제외
      const isPostalReceipt = allTexts.includes('우체국') || allTexts.includes('우편') || allTexts.includes('등기');
      const isInsuranceDoc = docTypes.some(d => d === '보험관련') || rec.purpose.includes('보험') || rec.purpose.includes('4대보험');
      // 우체국 영수증 "기프트카드잔액" 또는 보험문서 테이블 OCR 오인식 제외
      if (!isPostalReceipt && !isInsuranceDoc) {
...
function checkIngunbi(rec: Record, j: Judgment): void {
  ...
  // 필수 증빙: 급여명세서 또는 수당지급조서 (겸임수당/시간외수당/연가수당 등)
  const isAllowance = rec.purpose.includes('수당') || rec.purpose.includes('연가') || rec.purpose.includes('겸임') ||
    rec.purpose.includes('시간외') || rec.purpose.includes('명절') || rec.purpose.includes('휴가비');
  if (docTypes.some(d => ['급여명세', '이체증', '통장사본'].includes(d))) {
    j.checks.push('인건비 증빙(급여명세/이체증) 확인');
  } else if (isAllowance && docTypes.some(d => ['수당지급조서', '지출결의서', '입금의뢰서'].includes(d))) {
    j.checks.push('인건비(수당) 증빙(수당지급조서) 확인');
  } else {
...
// ===== 메인 =====
async function main() {
  console.log('=== 경기도의료원 안성병원 187건 검증 ===\n');
  ...
  for (const r of results) {
    if (r.judgment.status !== '적정') {
      ...
      console.log(`  파일: ${r.expandDocTypes(rec.files).join(', ')}`);
    }
  }
  ...
  const outPath = `${RESULT_DIR}/안성병원_검증결과_${ts}.xlsx`;
  fs.mkdirSync(RESULT_DIR, { recursive: true });
  await wb.xlsx.writeFile(outPath);
  console.log(`결과 파일: ${outPath}`);

  // 6. 상세 JSON 저장
  const jsonOut = `${RESULT_DIR}/안성병원_검증상세_${ts}.json`;
