/**
 * 한림성심대 272건 검증 프레임워크 적용
 * hallym-data.json → 비목별 규칙 적용 → 판단 결과 JSON + 엑셀
 */
import fs from 'fs';
import ExcelJS from 'exceljs';

const DATA_JSON = '/mnt/c/projects/e-naradomum-rpa/hallym-data.json';
...
function categorize(rec: Record): string {
  ...
  if (sub.includes('일반수용비') || cat.includes('운영비')) {
    ...
    if (purpose.includes('4대보험') || purpose.includes('보험금') || item.includes('보험')) return '복리후생비';
...
function checkIlbansuyongbi(rec: Record, j: Judgment): void {
  ...
  const isService = purpose.includes('용역') || purpose.includes('수당') || purpose.includes('인쇄') ||
    ...
    purpose.includes('배송') || purpose.includes('수수료');
...
function checkIncentive(rec: Record, j: Judgment): void {
  ...
  // 증빙 확인
  if (docTypes.some(d => ['지출결의서', '인센티브', '입금의뢰서'].includes(d))) {
...
function checkBokri(rec: Record, j: Judgment): void {
  // 4대보험 - 대부분 적정
  const docTypes = rec.files.map(f => f.docType);
  if (docTypes.some(d => ['지출결의서', '보험관련', '기안문'].includes(d))) {
    j.checks.push('복리후생비 증빙 확인');
  }
}
...
function checkBimokFit(rec: Record, j: Judgment): void {
  ...
  // 일반수용비인데 50만원 이상 (자산취득비 해당) - 인쇄/제작/용역/수당/사용료 제외
  if (cat === '일반수용비' && rec.totalAmount >= 500000) {
    const serviceKws = ['수당', '인센티브', '사용료', '용역', '인쇄', '제작', '사례금', '사례비', '원천징수', '배송', '수수료'];
...
async function main() {
  ...
  const outPath = `${RESULT_DIR}/한림성심대_검증결과_${ts}.xlsx`;
  fs.mkdirSync(RESULT_DIR, { recursive: true });
  await wb.xlsx.writeFile(outPath);
  console.log(`결과 파일: ${outPath}`);

  // 6. 상세 JSON 저장
  const jsonOut = `${RESULT_DIR}/한림성심대_검증상세_${ts}.json`;
