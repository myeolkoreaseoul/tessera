/**
 * 공통 데이터 수집 모듈
 * - 소스 엑셀 읽기 + 다운로드 폴더 OCR → JSON 출력
 *
 * 사용법:
 *   const collect = require('./lib/collect-generic');
 *   await collect.run({ sourceExcel, downloadDir, outputJson });
 *
 *   또는 CLI:
 *   node lib/collect-generic.js --excel=소스.xlsx --downloads=downloads/바이오링크 --output=biolink-data.json
 */
const { execSync } = require('child_process');
const fs = require('fs');
const path = require('path');
const ExcelJS = require('exceljs');
const { formatDate, parseNumber } = require('../../utils');
...
async function extractPdfText(filePath) {
  ...
  } catch { /* pdf-parse failed, fall through to OCR */ }
  // OCR 폴백
...
// ── 엑셀 읽기 (e나라도움 사업집행내역 표준 포맷) ──

async function readSourceExcel(sourceExcel) {
  const wb = new ExcelJS.Workbook();
...
async function collectFiles(records, downloadDir) {
  const EXTS = ['.pdf', '.xlsx', '.xls', '.hwp', '.jpg', '.jpeg', '.png', '.pptx'];
  let done = 0;

  for (const rec of records) {
    const dlDir = path.join(downloadDir, `r${rec.rowNum}`);
    ...
    for (const fn of fileNames) {
      ...
      rec.files.push({ name: fn, text: text.substring(0, 12000) });
    }
...
async function run({ sourceExcel, downloadDir, outputJson }) {
  console.log('[1/3] 소스 엑셀 읽기...');
  const records = await readSourceExcel(sourceExcel);
  console.log(`  ${records.length}건 로드`);

  console.log('[2/3] 다운로드 파일 OCR...');
  await collectFiles(records, downloadDir);

  console.log('[3/3] JSON 저장...');
  fs.writeFileSync(outputJson, JSON.stringify(records, null, 2), 'utf-8');
  const sizeMB = (fs.statSync(outputJson).size / 1024 / 1024).toFixed(1);
  console.log(`  ${outputJson} (${sizeMB}MB)`);
...
if (require.main === module) {
  ...
  const sourceExcel = getArg('--excel=');
  const downloadDir = getArg('--downloads=');
  const outputJson = getArg('--output=');

  if (!sourceExcel || !downloadDir || !outputJson) {
    console.log('사용법: node lib/collect-generic.js --excel=소스.xlsx --downloads=downloads/기관명 --output=기관명-data.json');
    process.exit(1);
  }

  run({ sourceExcel, downloadDir, outputJson }).catch(console.error);
}
