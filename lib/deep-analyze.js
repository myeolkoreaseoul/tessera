...
function loadRules(legalBasis) {
  ...
  if (fs.existsSync(commonDir)) {
    for (const f of fs.readdirSync(commonDir).filter(f => f.endsWith('.js')).sort()) {
      rules.push(require(path.join(commonDir, f)));
    }
  }

  // 2) 법령별 (보조금/ 또는 혁신법/)
  if (legalBasis) {
    const basisDir = path.join(rulesDir, legalBasis);
    if (fs.existsSync(basisDir)) {
      for (const f of fs.readdirSync(basisDir).filter(f => f.endsWith('.js')).sort()) {
        rules.push(require(path.join(basisDir, f)));
      }
    }
  }
