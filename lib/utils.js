...
async function connectBrowser(port = 9444) {
  // Electron 모드: 메인 프로세스에서 이미 열린 브라우저에 CDP로 연결
  if (process.env.TESSERA_MODE === 'electron') {
    const browserProvider = require('./browser-provider');
    return browserProvider.connectLocal(port);
  }
  // CDP 모드: 원격 Chrome 연결 (기존 동작)
  const host = process.env.CDP_HOST || '100.87.3.123';
  const cdpTimeout = Number(process.env.CDP_CONNECT_TIMEOUT_MS || 120000);
  const browser = await chromium.connectOverCDP(`http://${host}:${port}`, { timeout: cdpTimeout });
