/**
 * 바이오링크 54건 전체 파이프라인
 * Phase 1: 그리드 데이터 추출 (페이지네이션 포함)
 * Phase 2: 첨부파일 다운로드
 * Phase 3: OCR
 * Phase 4: biolink-data.json 저장
 */
// Playwright CDP dialog race condition 방지
process.on('unhandledRejection', (err) => {
  if (err && err.message && err.message.includes('No dialog is showing')) return; // 무시
...
const BASE_DIR = path.join(__dirname, 'downloads/biolink');
const OUTPUT = path.join(__dirname, 'biolink-data.json');
...
async function extractAllGridData(page) {
  ...
  while (true) {
    const pageData = await page.evaluate(() => {
      ...
      // totalNum from first row or DOM
    ...
    const hasNext = await page.evaluate((pn) => {
      const link = document.querySelector(`a[onclick*="DD001002QGridObj"][onclick*="${pn}"]`) ||
                   document.querySelector(`a[href*="page(${pn})"]`);
      if (link) { link.click(); return true; }
      // IBSheet 직접 호출
      try { DD001002QGridObj.page(pn); return true; } catch { return false; }
    }, pageNum);
...
async function main() {
  ...
  const page = context.pages().find(p =>
    p.url().includes('gosims') && !p.url().includes('getDB003002SView')
  );
  if (!page) { console.log('e나라도움 페이지 없음!'); return; }
  console.log('메인 URL:', page.url());

  // 다이얼로그 자동 처리 (Playwright 레벨)
  ...
  if (total > 20) {
    await page.evaluate(() => {
      try { DD001002QGridObj.page(1); } catch {}
    });
  ...
  // ── Phase 4: JSON 저장 ──
  // atchmnflId, excutId는 judge에 불필요하므로 제거
  ...
  console.log('\n=== 파이프라인 Phase 1-4 완료 ===');
  console.log('다음: node lib/judge-digital-healthcare.js --data=biolink-data.json --name=바이오링크');
}
