...
export class ExecutionScraper {
   ...
   */
  async getExecutionList(): Promise<ExecutionRecord[]> {
    logger.info('집행내역 목록 수집 중...');
    ...
    try {
      ...
      for (const container of scrollContainers) {
        const scrollHeight = await container.evaluate((el: Element) => (el as HTMLElement).scrollHeight);
      ...
      if (!scrollContainer) {
        ...
        if (!scrollContainer) {
          logger.warn('스크롤 컨테이너를 찾을 수 없습니다. 기본 방식으로 시도...');
      ...
      const clientHeight = await scrollContainer.evaluate((el: Element) => el.clientHeight);
      // 가상 스크롤에서는 한 행 높이(약 30px) 기준으로 5행씩 스크롤
      const rowHeight = 30;
      const scrollStep = rowHeight * 5; // 약 5행씩 스크롤 (wheel 이벤트와 함께 사용)

      logger.debug(`스크롤 높이: ${scrollHeight}, 클라이언트 높이: ${clientHeight}, 스크롤 단위: ${scrollStep}`);
      ...
      for (let attempt = 0; attempt < maxScrollAttempts; attempt++) {
        ...
        if (totalCount > 0) {
          logger.progress(records.length, totalCount, `수집 중`);
        } else {
          logger.debug(`${records.length}건 수집됨`);
        }
        ...
        // 스크롤 다운 (wheel 이벤트 사용)
        await scrollContainer.evaluate((el: Element, step: number) => {
          // scrollTop 변경
          el.scrollTop += step;
   ...
   * 현재 보이는 행들 수집
   */
  private async collectVisibleRows(seenIds: Set<string>): Promise<ExecutionRecord[]> {
    const newRecords: ExecutionRecord[] = [];
    ...
    const rowData = await this.page.evaluate(() => {
      ...
      for (let rowIdx = 0; rowIdx < maxRows; rowIdx++) {
        ...
        if (allCells.length < 5) continue; // 데이터 행이 아님
    ...
    for (const data of rowData) {
      if (seenIds.has(data.uniqueKey)) continue;
   ...
   */
  private parseRowData(cells: string[], rowNumber: number): ExecutionRecord | null {
    try {
      ...
      for (const cell of cells) {
        if (/^\d{4}-\d{2}-\d{2}$/.test(cell)) {
          if (!executionDate) executionDate = cell;
          else if (!writeDate) writeDate = cell;
        }
      }
   ...
   * 금액 파싱 (숫자 형식 찾기)
   */
  private parseAmount(cells: string[]): number {
    for (const cell of cells) {
      // 금액 형식 (콤마 포함 숫자)
   ...
   * 기본 방식으로 집행내역 수집 (폴백)
   */
  private async getExecutionListBasic(): Promise<ExecutionRecord[]> {
    const records: ExecutionRecord[] = [];
  ...
  private async parseTableRow(
    ...
    try {
      ...
      const getNumber = async (cell: any): Promise<number> => {
      ...
      // 첨부파일 건수 (보기 링크 옆 숫자)
      const attachmentCountText = await row.$eval(
        'td:has(a:has-text("보기"))',
        (el: Element) => {
   ...
   * 엑셀 다운로드 버튼 클릭
   */
  async downloadExcel(): Promise<string | null> {
    ...
    try {
      ...
      if (!excelButton) {
        logger.warn('엑셀 다운로드 버튼을 찾을 수 없습니다.');
        return null;
      }
      ...
      logger.success(`엑셀 다운로드 완료: ${suggestedFilename}`);
      return savePath;

    } catch (error) {
      logger.error('엑셀 다운로드 실패', error);
      return null;
    }
  }
   ...
   */
  async scrollToRightColumns(): Promise<void> {
    try {
      // 테이블 컨테이너 찾기
      ...
      if (tableContainer) {
        // 오른쪽으로 스크롤
        await tableContainer.evaluate((el: Element) => {
