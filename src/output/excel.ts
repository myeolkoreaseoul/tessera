...
export class ResultExporter {
  ...
  async exportResults(
    ...
    logger.info('결과 엑셀 생성 중...');

    // 데이터 준비
    const rows: ResultRow[] = records.map((record, i) => ({
      record,
      result: results[i],
    }));
    ...
    logger.success(`결과 파일 생성: ${filename}`);
   ...
   * 전체 목록 시트
   */
  private createAllSheet(rows: ResultRow[]) {
   ...
   * 부적정/확인필요 시트
   */
  private createIssueSheet(rows: ResultRow[]) {
    const sheet = this.workbook.addWorksheet('부적정_확인필요');

    // 부적정/확인필요 건만 필터
    const issueRows = rows.filter(
      r => r.result.judgment === 'inappropriate' || r.result.judgment === 'needsReview'
    );
   ...
   * 통계 시트
   */
  private createStatsSheet(rows: ResultRow[]) {
   ...
   * 판단 결과 텍스트 변환
   */
  private getJudgmentText(judgment: Judgment): string {
    switch (judgment) {
      case 'appropriate':
        return '적정';
      case 'inappropriate':
   ...
   * 행 스타일 적용
   */
  private applyRowStyle(row: ExcelJS.Row, judgment: Judgment) {
    let bgColor: string;
